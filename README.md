...
abre tu termininal termux y ejecuta los siguientes comandos:

```bash
pkg update -y && pkg upgrade -y && apt update -y && apt upgrade -y
```

### Paso 2 - Despues toca instalar algunas dependencias manualmente :

```bash
pkg install git -y && apt install which fish git curl -y 
```

## Paso 3 -  Clona un repositorio para que se instalen todos los paquetes necesarios :

```bash
git clone https://github.com/Arturo254/Basic-Tools && cd Basic-Tools && chmod +x Tool.sh && bash Tool.sh 
```

*si en algun momento se deitene y piede alguna confirmacion como __Y/N__ solo ingresa la letra __Y__  y continuara y espera a que termine*

## Paso 4 - Instalar oh my fish :

```bash
curl https://raw.githubusercontent.com/oh-my-fish/oh-my-fish/master/bin/install | fish
```

## Paso 5 - Instalar el tema zish :

###### zish es un tema de fish para terminales linux

```bash
omf install zish
```

## Paso 6 - Actualizar el banner :

```bash
rm -d -f $PREFIX/etc/modt.sh bash.bashrc
git clone https://github.com/Arturo254/bash.bashrc && cd bash.bashrc/ && mv bash.bashrc $PREFIX/etc 
```

*con esto se remueve el banner anterior de termux t se reemplaza por uno mas bonito*

## paso 7 - Modificar el tema zish :
